...
import m$1 from 'react-dom/server';
...
async function P$1({ request: n, event: t }) {
  const s = new AbortController(), i = s.signal, v = () => s.abort();
  t.node.req.on("close", v);
  const h = n.method, g = new URL(n.url, "http://localhost:3000"), _ = new RegExp(`${D("/_server")}/([^/?#]+)`), y = g.pathname.match(_), o = y ? y[1] : null, c = Object.fromEntries(g.searchParams.entries()), m = "createServerFn" in c, E = "raw" in c;
  if (typeof o != "string") throw new Error("Invalid server action param for serverFnId: " + o);
  const w = d[o];
  if (!w) throw console.log("serverFnManifest", d), new Error("Server function info not found for " + o);
  let l;
  if (l = await w.importer(), !l) throw console.log("serverFnManifest", d), new Error("Server function module not resolved for " + o);
  const a = l[w.functionName];
  ...
  const T = ["multipart/form-data", "application/x-www-form-urlencoded"], f = await (async () => {
    try {
      let e = await (async () => {
        if (n.headers.get("Content-Type") && T.some((r) => {
          var S;
          return (S = n.headers.get("Content-Type")) == null ? void 0 : S.includes(r);
        })) return M(h.toLowerCase() !== "get", "GET requests with FormData payloads are not supported"), await a(await n.formData(), i);
        if (h.toLowerCase() === "get") {
          let r = c;
          return m && (r = c.payload), r = r && startSerializer.parse(r), await a(r, i);
        }
        const p = await n.text(), F = startSerializer.parse(p);
        return m ? await a(F, i) : await a(...F, i);
      })();
      return e.result instanceof Response ? e.result : !m && (e = e.result, e instanceof Response) ? e : isRedirect(e) || isNotFound(e) ? C(e) : new Response(e !== void 0 ? startSerializer.stringify(e) : void 0, { status: getResponseStatus(getEvent()), headers: { "Content-Type": "application/json" } });
    } catch (e) {
